...
export function useFlowController({
  ...
  const [currentDirection, setCurrentDirection] = useState<Direction>(initialDirection);
  ...
  const recalculateLayout = useCallback((direction: Direction, collapsed: Set<string>) => {
    ...
    const visibleNodes = layoutedNodes.filter(node => !collapsed.has(node.id));
  ...
  const toggleNodeChildren = useCallback((nodeId: string) => {
    ...
    setCollapsedNodes((prev) => {
      ...
      const anyImmediateChildVisible = immediateChildren.some(id => !prev.has(id));
  ...
  }, [descendantsCache, immediateChildrenCache, currentDirection, recalculateLayout]);
